...
def add_poker_game(game_data_path: str, dest_path: str, exclude_list=[]):
    ...
    net_winnings_by_player = {key: value for key, value in net_winnings_by_player.items() if key not in exclude_list}
...
def print_winnings_of_game(ledger_path: str):
    ...
    net_winnings_by_player = (game_data.groupby('player_nickname')['net'].sum() / 100).to_dict()
    sorted_winnings = dict(sorted(net_winnings_by_player.items(), key=lambda item: item[1], reverse=True))
...
if __name__ == "__main__":
    ...
    add_poker_game('ledgers/ledger10_31(2).csv', 'data.json', ['grant lumkong'])
    # unique_nicknames('ledgers')
    # reset_net_games_played('data.json')
    # sort_days_list('data.json')
    # add_all_games('ledgers', 'data.json')
    # print_winnings_of_game('ledgers/ledger10_31(2).csv')
    print('\ndone')